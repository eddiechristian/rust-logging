...
async fn demonstrate_deletion_patterns() {
    ...
    let (checked, removed) = DeviceCacheManager::iterate_and_remove_with_logging(|mac_address, entry| {
        // Remove mobile devices with specific MAC patterns
        entry.device_id.contains("mobile") && mac_address.to_string().contains("77:88")
    });
    info!("Custom removal: checked {} entries, removed {} entries", checked, removed);
