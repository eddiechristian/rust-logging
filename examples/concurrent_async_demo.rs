...
async fn main() {
                ...
                match DeviceCacheManager::get_device_entry_by_mac_str(&target_mac) {
                    Some(entry) => {
                        match DeviceCacheManager::update_cache_entry_by_mac_str(&target_mac, entry) {
                            ...
                            Err(e) => {
                                eprintln!("[UPDATER] Failed to update async MAC {}: {}", target_mac, e);
                            }
                        }
                    }
                    None => {
                        info!("[UPDATER] Async device with MAC {} not found yet, skipping", target_mac);
                    }
                }
            }
        }
    ...
    let monitor_task = tokio::spawn(async move {
        ...
        while running_monitor.load(Ordering::Relaxed) {
            ...
            if !high_activity.is_empty() {
                info!("[MONITOR] High-activity devices (≥3 heartbeats): {}", high_activity.len());
            ...
            info!("[MONITOR] Recently active devices (last 5s): {}", recent_devices.len());
        }
    ...
    let collection_task = tokio::spawn(async move {
        ...
        for i in 0..5 {
            ...
            info!("  Found {} devices with IP starting with '10.'", ip_pattern_devices.len());
            ...
            info!("  Found {} async devices (MAC starts with 'aa:bb:cc')", async_devices.len());
            
            // Collect devices by device name pattern
            let named_devices = DeviceCacheManager::collect_entries_by_device_pattern("async_device");
            info!("  Found {} devices with 'async_device' in name", named_devices.len());
            
            // Show some stats
            if !async_devices.is_empty() {
                let total_heartbeats: u64 = async_devices.iter().map(|(_, entry)| entry.heartbeat_count).sum();
                let avg_heartbeats = total_heartbeats as f64 / async_devices.len() as f64;
                info!("  Average heartbeats per async device: {:.2}", avg_heartbeats);
            }
        }
    ...
    info!("Average heartbeats per device: {:.2}", 
    ...
    let top_updated = DeviceCacheManager::collect_entries_matching(|_, entry| {
