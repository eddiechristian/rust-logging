    ...
    for i in 1..=4 {
        ...
        for pattern in mac_patterns {
            ...
            if let Some(entry) = DeviceCacheManager::get_device_entry_by_mac_str(&mac_str) {
                for _ in 0..5 { // Give them 5 extra heartbeats
                    let _ = DeviceCacheManager::update_cache_entry_by_mac_str(&mac_str, entry.clone());
                }
            }
        }
    }
...
async fn demonstrate_collection_functions() {
    ...
    info!("Found {} devices on development network:", dev_network.len());
    ...
    info!("Found {} high-activity devices (≥3 heartbeats):", high_activity.len());
    ...
    info!("Found {} devices matching complex criteria:", complex_match.len());
    ...
    info!("Found {} VMware devices (MAC starts with 00:50:56):", vmware_devices.len());
    ...
    info!("  Collection function: found {} items in {:?}", collected.len(), collection_time);
    info!("  Manual iteration: found {} items in {:?}", iteration_count, iteration_time);
