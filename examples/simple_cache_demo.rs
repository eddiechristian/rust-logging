...
async fn main() {
    ...
    let async_task = tokio::spawn(async {
        for i in 0..3 {
            ...
            if let Err(e) = DeviceCacheManager::add_device_entry(device_id.clone(), mac.clone(), ip, Some(443)) {
    ...
    let thread_handle = thread::spawn(|| {
        for i in 0..3 {
            ...
            if let Err(e) = DeviceCacheManager::add_device_entry(device_id.clone(), mac.clone(), ip, Some(8080)) {
    ...
    let updated_count = DeviceCacheManager::update_all_entries(|mac_address, entry| {
        info!("Batch updating MAC {}: device_id={}", mac_address, entry.device_id);
